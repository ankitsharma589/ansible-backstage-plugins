...
# Note: yarn tsc is run at workspace root level in pack.sh before processing plugins
run_cmd yarn install
run_cmd yarn build

# Run export-dynamic using npx to ensure rhdh-cli is found from workspace root
EXPORT_CMD=$(< package.json jq -r '.scripts["export-dynamic"]' | sed 's/^rhdh-cli/npx @red-hat-developer-hub\/cli/')
echo "Running export-dynamic: $EXPORT_CMD"
if ! eval "$EXPORT_CMD"; then
  echo "export-dynamic failed in $pluginDir"
  popd > /dev/null
  exit 1
fi
